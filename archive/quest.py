    ...
    def update(self,intensity,response):
        ...
        if self.updatePdf:
            ...
            if not np.allclose(ii,iii):
                raise ValueError('truncation error')
            self.pdf = self.pdf*self.s2[response,iii]
            plt.plot(self.x,self.s2[response,iii])
            if self.normalizePdf:
                self.pdf=self.pdf/np.sum(self.pdf)
...
def demo():
    ...
    for k in range(trialsDesired):
        ...
        plt.plot([tTest,tTest],[0,max(q.pdf/max(q.pdf))])
        plt.plot(q.x2,q.s2[0,:],label='failure')
        plt.plot(q.x2,q.s2[1,:],label='success')
        plt.legend()
...
plt.scatter(q.intensity, q.response)
plt.plot(q.pdf)
